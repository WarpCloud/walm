...
func ListTenants(request *restful.Request, response *restful.Response) {
	...
	if err != nil {
		api.WriteErrorResponse(response, -1, fmt.Sprintf("failed to list tenants : %s", err.Error()))
		return
	}
...
func CreateTenant(request *restful.Request, response *restful.Response) {
	...
	if err != nil {
		api.WriteErrorResponse(response, -1, fmt.Sprintf("failed to read tenant params : %s", err.Error()))
		return
	}
	err = tenant.CreateTenant(tenantName, tenantParams)
	if err != nil {
		api.WriteErrorResponse(response, -1, fmt.Sprintf("failed to create tenant : %s", err.Error()))
		return
	}
}
...
func GetTenant(request *restful.Request, response *restful.Response) {
	...
	if err != nil {
		if walmerr.IsNotFoundError(err) {
			api.WriteNotFoundResponse(response, -1, fmt.Sprintf("tenant %s is not found", tenantName))
			return
		}
		api.WriteErrorResponse(response, -1, fmt.Sprintf("failed to get tenant : %s", err.Error()))
		return
	}
...
func DeleteTenant(request *restful.Request, response *restful.Response) {
	...
	if err != nil {
		api.WriteErrorResponse(response, -1, fmt.Sprintf("failed to delete namespace %s: %s" , tenantName, err.Error()))
		return
	}
...
func UpdateTenant(request *restful.Request, response *restful.Response) {
	...
	if err != nil {
		api.WriteErrorResponse(response, -1, fmt.Sprintf("failed to update tenant : %s", err.Error()))
		return
	}
}
