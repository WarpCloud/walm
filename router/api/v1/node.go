...
func GetNodes(request *restful.Request, response *restful.Response) {
	...
	if err != nil {
		api.WriteErrorResponse(response, -1, fmt.Sprintf("failed to get nodes: %s", err.Error()))
		return
	}
...
func GetNode(request *restful.Request, response *restful.Response) {
	...
	if nodeName == "" {
		api.WriteErrorResponse(response, -1, fmt.Sprintf("node name can not be empty"))
		return
	}
	node, err := adaptor.GetDefaultAdaptorSet().GetAdaptor("Node").(*adaptor.WalmNodeAdaptor).GetResource("", nodeName)
	if err != nil {
		api.WriteErrorResponse(response, -1, fmt.Sprintf("failed to get node: %s", err.Error()))
		return
	}
	if node.GetState().Status == "NotFound" {
		api.WriteNotFoundResponse(response, -1, fmt.Sprintf("node %s is not found", nodeName))
		return
	}
...
func LabelNode(request *restful.Request, response *restful.Response) {
	...
	if err != nil {
		api.WriteErrorResponse(response, -1, fmt.Sprintf("failed to label node %s: %s", nodeName, err.Error()))
		return
	}
}
...
func AnnotateNode(request *restful.Request, response *restful.Response) {
	...
	if err != nil {
		api.WriteErrorResponse(response, -1, fmt.Sprintf("failed to annotate node %s: %s", nodeName, err.Error()))
		return
	}
}
