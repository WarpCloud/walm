...
var _ = Describe("K8sCache", func() {
	...
	BeforeEach(func() {
		By("create namespace")
		namespace, err = framework.CreateRandomNamespace("k8sCacheTest")
		Expect(err).NotTo(HaveOccurred())
		stopChan = make(chan struct{})
		k8sCache = informer.NewInformer(framework.GetK8sClient(), framework.GetK8sReleaseConfigClient(), 0, stopChan)

		storageClass, err = framework.CreateStorageClass(namespace, "test-storageclass")
		Expect(err).NotTo(HaveOccurred())
	})
	...
	It("test get resource & get resource set", func() {
		...
		secret, err := framework.CreateSecret(namespace, "test-secret")
		Expect(err).NotTo(HaveOccurred())
