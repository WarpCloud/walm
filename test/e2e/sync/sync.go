...
var _ = Describe("Sync", func() {
	...
	It("test sync releases & projects", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(rc).To(Equal("{\"name\":\"testnm\",\"repoName\":\"\",\"configValues\":null,\"version\":0,\"namespace\":\"testns\",\"dependencies\":null,\"chartName\":\"\",\"chartVersion\":\"\",\"chartAppVersion\":\"\",\"releaseResourceMetas\":null,\"computedValues\":null,\"metaInfoValues\":null,\"manifest\":\"\"}"))
		...
		rc, err = redisClient.HGet(testRcKey, "testns/testnm").Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(rc).To(Equal("{\"name\":\"testnm\",\"repoName\":\"\",\"configValues\":null,\"version\":0,\"namespace\":\"testns\",\"dependencies\":null,\"chartName\":\"testct\",\"chartVersion\":\"\",\"chartAppVersion\":\"\",\"releaseResourceMetas\":null,\"computedValues\":null,\"metaInfoValues\":null,\"manifest\":\"\"}"))
		rt, err = redisClient.HGet(testRtKey, "testns/testnm").Result()
		Expect(err).NotTo(HaveOccurred())
		Expect(rt).To(Equal("{\"name\":\"testnm\",\"namespace\":\"testns\",\"latestReleaseTaskSignature\":null}"))
		pt, err = redisClient.HGet(testPtKey, "testns/testpj").Result()
