	...
	Describe("test install release", func() {
		...
		Describe("need fixed namespaces", func() {
			...
			It("test list & delete release", func() {
				...
				_, err := helm.InstallOrCreateRelease(fixedNamespace1, releaseRequest, tomcatChartFiles, false, false, nil, nil)
				Expect(err).NotTo(HaveOccurred())

				_, err = helm.InstallOrCreateRelease(fixedNamespace2, releaseRequest, tomcatChartFiles, false, false, nil, nil)
				Expect(err).NotTo(HaveOccurred())
				...
				releaseCache1 := getReleaseCache(releaseCaches, fixedNamespace1, "tomcat-test")
				Expect(releaseCache1).NotTo(BeNil())
				releaseCache2 := getReleaseCache(releaseCaches, fixedNamespace2, "tomcat-test")
				Expect(releaseCache2).NotTo(BeNil())

				assertReleaseCacheBasic(releaseCache1, fixedNamespace1, "tomcat-test", "", "tomcat",
					"0.2.0", "7", 1)

				assertYamlConfigValues(releaseCache1.ComputedValues, tomcatComputedValues)

				manifest := fmt.Sprintf("\n---\napiVersion: v1\nkind: Service\nmetadata:\n  labels:\n    app: tomcat\n    chart: tomcat-0.2.0\n    heritage: Helm\n    release: tomcat-test\n  name: tomcat-test\n  namespace: %s\nspec:\n  ports:\n  - name: http\n    port: 80\n    protocol: TCP\n    targetPort: 8080\n  selector:\n    app: tomcat\n    release: tomcat-test\n  type: NodePort\n\n---\napiVersion: apps/v1beta2\nkind: Deployment\nmetadata:\n  labels:\n    app: tomcat\n    chart: tomcat-0.2.0\n    heritage: Helm\n    release: tomcat-test\n  name: tomcat-test\n  namespace: %s\nspec:\n  replicas: 1\n  selector:\n    matchLabels:\n      app: tomcat\n      release: tomcat-test\n  template:\n    metadata:\n      labels:\n        app: tomcat\n        release: tomcat-test\n    spec:\n      containers:\n      - image: tomcat:7.0\n        imagePullPolicy: Always\n        livenessProbe:\n          httpGet:\n            path: /sample\n            port: 8080\n          initialDelaySeconds: 60\n          periodSeconds: 30\n        name: tomcat\n        ports:\n        - containerPort: 8080\n          hostPort: 8009\n        readinessProbe:\n          failureThreshold: 6\n          httpGet:\n            path: /sample\n            port: 8080\n          initialDelaySeconds: 60\n          periodSeconds: 30\n        resources:\n          limits:\n            cpu: 0.2\n            memory: 200Mi\n          requests:\n            cpu: 0.1\n            memory: 100Mi\n        volumeMounts:\n        - mountPath: /usr/local/tomcat/webapps\n          name: app-volume\n      initContainers:\n      - command:\n        - sh\n        - -c\n        - cp /*.war /app\n        image: ananwaresystems/webarchive:1.0\n        imagePullPolicy: Always\n        name: war\n        volumeMounts:\n        - mountPath: /app\n          name: app-volume\n      volumes:\n      - emptyDir: {}\n        name: app-volume\n\n---\napiVersion: apiextensions.transwarp.io/v1beta1\nkind: ReleaseConfig\nmetadata:\n  creationTimestamp: null\n  name: tomcat-test\n  namespace: %s\nspec:\n  chartAppVersion: \"7\"\n  chartImage: \"\"\n  chartName: tomcat\n  chartVersion: 0.2.0\n  configValues: {}\n  dependencies: null\n  dependenciesConfigValues: {}\n  outputConfig: {}\n  repo: \"\"\nstatus: {}\n",
					fixedNamespace1, fixedNamespace1, fixedNamespace1)
				Expect(releaseCache1.Manifest).To(Equal(manifest))

				Expect(releaseCache1.ReleaseResourceMetas).To(Equal(getTomcatChartDefaultReleaseResourceMeta(fixedNamespace1, "tomcat-test")))

				mataInfoParams := getTomcatChartDefaultMetaInfoParams()
				Expect(releaseCache1.MetaInfoValues).To(Equal(mataInfoParams))

				By("delete release")
				err = helm.DeleteRelease(fixedNamespace1, "tomcat-test")
				Expect(err).NotTo(HaveOccurred())

				err = helm.DeleteRelease(fixedNamespace1, "not-existed")
				Expect(err).NotTo(HaveOccurred())
