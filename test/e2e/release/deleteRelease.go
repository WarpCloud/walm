...
var _ = Describe("Release", func() {
	...
	Describe("delete project release", func() {
		It("delete project release success", func() {

			By("delete release")
			err := helm.GetDefaultHelmClient().DeleteRelease(namespace, releaseName, true)
			Expect(err).NotTo(HaveOccurred())

			By("get release info fail, delete success")
			_, err = helm.GetDefaultHelmClient().GetRelease(namespace, releaseName)
			if err != nil {
				fmt.Fprintln(GinkgoWriter, "delete project release success")
			}
		})
	})
})
