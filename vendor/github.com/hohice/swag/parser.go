...
func (parser *Parser) parseStruct(pkgName string, field *ast.Field) (properties map[string]spec.Schema) {
	...
	if _, ok := parser.TypeDefinitions[pkgName][structField.schemaType]; ok { // user type field
		...
		} else if structField.arrayType[0:2] == "&{" { //if item is ref --ref it
			array := strings.Split(structField.arrayType[2:len(structField.arrayType)-1], " ")
			if len(array) > 1 {
				refPkgName, arrayType := array[0], array[1]
				parser.ParseDefinition(refPkgName, parser.TypeDefinitions[refPkgName][arrayType], arrayType)
				properties[structField.name] = spec.Schema{
					SchemaProps: spec.SchemaProps{
						Type:  []string{structField.schemaType},
						Items: &spec.SchemaOrArray{Schema: &spec.Schema{SchemaProps: spec.SchemaProps{Ref: spec.Ref{Ref: jsonreference.MustCreateRef("#/definitions/" + refPkgName + "." + arrayType)}}}},
					},
				}
			} else {
				arrayType := array[0]
