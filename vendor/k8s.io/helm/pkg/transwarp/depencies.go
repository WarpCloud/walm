...
func CheckDepencies(chartRequested *chart.Chart, depLinks map[string]interface{}) (error) {
	...
	if dep == nil || len(dep) == 0 {
		fmt.Fprintf(os.Stdout, "WARNING: Cannot found depencies in transwarp-app-yaml")
		return nil
	}

	if appDepencies == nil || len(appDepencies) == 0 {
		fmt.Fprintf(os.Stdout, "WARNING: Cannot found depencies requires in transwarp-app-yaml")
		return nil
	}
	...
	for k, v := range depLinks {
		...
		if found == false {
			fmt.Fprintf(os.Stdout, "WARNING: links %s=%s not defined in transwarp-app-yaml. skip...\n", k, v)
			continue
		}
	...
	if found == false {
		fmt.Fprintf(os.Stdout, "WARNING: All links is not  defined in transwarp-app-yaml. skip...\n")
		return nil
	}
...
func resolveServiceAnnotations(serviceAnnotations map[string]string, annotations *map[string]interface{}) error {
	...
	for _, val := range serviceAnnotations {
		...
		err := json.Unmarshal([]byte(val), &depenciesConfig)
		if err != nil {
			return err
		}
