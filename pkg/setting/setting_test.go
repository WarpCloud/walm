...
func TestEnvSettings(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "with flags set",
			args:    []string{"--home", "/foo", "--dbhost=here", "--debug"},
			home:    "/foo",
		...
		},
		{
			...
			args:    []string{},
			envars:  map[string]string{"WALM_HOME": "/bar", "WALM_HOST": "there"},
			home:    "/bar",
		...
		},
		{
			...
			args:    []string{"--home", "/foo", "--dbhost=here"},
			envars:  map[string]string{"WALM_HOME": "/bar", "WALM_HOST": "there", "WALM_PLUGIN": "glade", "WALM_DEBUG": "1"},
			home:    "/foo",
			plugins: "glade",
			host:    "here",
			debug:   true,
